...
const leaveGrp = (socket, grpName) => {
    let answer = prompt(`${socket.nickName} Are you sure you want to leave ${grpName} Group ❓❓ \n Answer with yes or no`);
    while (answer !== "yes" && answer !== "Yes" && answer !== "No" && answer !== "no") {
        alert("Invalid answer !!🙈🙈, Try again");
        answer = prompt(`${socket.nickName} Are you sure you want to leave ${grpName} Group ❓❓ \n Answer with yes or no`);
    }
...
grpSocket.on("connect", () => {
    console.log(`A new user connected to the group namespace and his id is ${grpSocket.id}`);
    grpSocket.emit("send grp id", grpID);
    grpSocket.nickName = prompt("What is your name ❓🤔");
    displayConnectionStatus(grpSocket.nickName);
    grpSocket.emit("send user name and grp name", grpSocket.nickName, grpID, grpName);
