...
grpNSP.on("connection", (grpSocket) => {
    ...
    grpSocket.on("send user name and grp name", (userName, grpID, grpName) => {
        ...
        console.log(`${grpSocket.nickName} has connected to Group ${grpName}, his ID is: ${socketId}`);
        ...
        grpNSP.to(grpID).emit("users list", grpNSP.adapter.rooms.get(grpID).connectedUsers);
